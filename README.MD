...
The premium analog supply capacitor is way oversized to help with very low bass. The output filter circuit is more complex than
factory to improve the channel response match.

## Board Features

By default the board is designed for balanced output, to be used with balanced amplifiers. 
It may be used unbalanced but the output voltage is 0 to 2V not -1V to 1V.

The TAD5242 is a high quality TI DAC chip. It supports a number of options (such as i2c control) 
that are not enabled on this specific board. The one available option is an SMD location for a resistor to 
